...
def qwerty():
        ...
        if (control_edit==0 or control_memory_mode==6):
            ...
            if (go_error==11):
                ...
                if (go_error==1):
                    ...
                    canvas.create_text(ras*10.5,ras*6.75,text="подумай еще",font="Verdana 7",justify=CENTER,fill="black")
                elif (go_error==5):
                    canvas.create_text(ras*10.5,ras*6.75,text="задание выполнено!",font="Verdana 7",justify=CENTER,fill="black")
            # for special version:
            if (control_memory_mode!=6):
    ...
    def click1(event):
        ...
        if (klik_flag==1):
                    ...
                    if (x>=ras*9.25 and x<=ras*11.75):
                        ...
                        if (control_memory_mode==0): # режим для развития памяти
                            ...
                            if (x>=ras*11.375 and y>=ras*6.25 and x<=ras*11.75 and y<=ras*6.625):
                                ...
                                out_window.update()
                                sleep(0.1)
                                # https://www.youtube.com/playlist?list=PL24VxeCr7LD1Z3Cm_VS4bLKthcGdkNWSD
            ...
            if (control_edit==0 or control_memory_mode==6): # вызов рисовалки виджетов
                if (len(history)>1):
                    control_protocol.clear()
                    control_protocol_flag = len(history) - 1
                    i = 0
