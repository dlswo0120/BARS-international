...
def qwerty():
    ...
    out_window = Tk()
    out_window.title('BARS 1.3')  # demonstration
    #window.geometry('1000x1000')
    #canvas = Canvas(out_window,width=9*ras,height=9*ras)
    canvas = Canvas(out_window,width=9*ras,height=10*ras,bg="white") # ,cursor="pencil"
        ...
        if (klik_flag==1):
            ...
            if ((x >= ras/2) and (x <= ras/2*17) and (y >= ras/2) and (y <= ras/2*17)): 
                ...
                if (control_edit==0):
                    ...
                    if (control_edit_checkers[1]==x and control_edit_checkers[0]==y):
                        if (area_monitor[y][x]==0):
                            area_monitor[y][x] = control_add
                        elif (area_monitor[y][x]/10<1):
                            area_monitor[y][x] = area_monitor[y][x] + 10
                        elif (area_monitor[y][x]%10==control_add):
                            if (control_add==1):
                                area_monitor[y][x] = 2
                            else:
                                area_monitor[y][x] = 1
                        ...
                        control_edit_checkers = [y,x]
                        if (area_monitor[y][x]==control_add):
                            area_monitor[y][x] = area_monitor[y][x] + 10
                        elif (area_monitor[y][x]==0):
                            area_monitor[y][x] = control_add
                        else:
                            area_monitor[y][x] = 0
            else:
