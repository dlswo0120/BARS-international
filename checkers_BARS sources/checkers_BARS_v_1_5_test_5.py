...
def qwerty():
    ...
    def vidget1():
        ...
        while (i<len(control_protocol)):
            ...
            #else:
            #    canvas.create_text(ras*9.6,ras*0.7+i*ras*0.25,text=".",font="Verdana 8",justify=CENTER,fill="black")
            if (mas[1]==-1):
            ...
            i = i + 1
        #i = 3
        #canvas.create_text(ras*9.5,ras*0.7,text=i,font="Verdana 8",justify=CENTER,fill="black")
        #canvas.create_text(ras*9.65,ras*0.7,text="1",font="Verdana 8",justify=CENTER,fill="black")
        #canvas.create_text(ras*9.5,ras*0.95,text="1",font="Verdana 8",justify=CENTER,fill="black")
    ...
    def click1(event):
        ...
        if (klik_flag==1):
            ...
            if (len(history)>1):
                ...
                if (int(history[control_protocol_flag][0]/100)%10==2):
                    control_protocol_flag = control_protocol_flag + 1
            ...
            if (len(go_operation)>0):
                qwertyuio=0
                #control_protocol.clear()
                #control_protocol_flag = len(history)
            else:
    ...
    def craft_protocol(a,b):
        ...
        global history
        i = 0
        while (a<=b and i<200): # i<200 на всякий пожарный
    ...
    def click3(event):
        ...
        if (x>=ras*9.25 and x<=11.75 and y>=ras*0.5 and y<=ras*6):
            qwertyu=0
        if (x>=ras*9.25 and x<=11.75 and y>=ras*6.5 and y<=ras*8):
            qwertyu=0
        print(y,x,d)
