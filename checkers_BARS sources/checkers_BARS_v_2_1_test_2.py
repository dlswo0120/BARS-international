...
def qwerty():
    ...
    def nostardamus(go_color):
        ...
        H = 4
        area_operation = deepcopy(area_monitor)
        generator_move(area_operation,go_color)
        i = 0
        go_operation_save = deepcopy(go_operation)
        while (i<len(go_operation_save)):
            #area_monitor1 = deepcopy(area_monitor)
            area_monitor1 = going1(area_monitor, go_operation_save[i])
            go.append([go_operation_save[i],evklid(area_monitor1, go_color)])
            #helper(area_monitor,H)
            i = i + 1
        #i = len(go_operation) - 1
        print_mas(go)
        print("---------------------")

    def evklid(area, go_color):
        global go_operation
        x = 0
        y = 0
        e = 0
