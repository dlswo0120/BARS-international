...
debuts = [["name",                           [[9, 0, 9, 0, 9, 0, 9, 0],
          ...
          ["жертва Кукуева",                 [[9, 2, 9, 2, 9, 2, 9, 2],
                                              [2, 9, 0, 9, 2, 9, 2, 9],
                                              [9, 2, 9, 0, 9, 0, 9, 2],
                                              [0, 9, 1, 9, 1, 9, 0, 9],
                                              [9, 0, 9, 0, 9, 0, 9, 0],
...
control_memory_mode = 0 # задания на память: 1,2 - просто тренажер, 3,4 - дебюты
...
def qwerty():
    ...
    out_window = Tk()
    out_window.title('BARS 2.2 demo')  # demonstration
    ...
    def  reboot_controller():
        ...
        if (control_edit==0):
            ...
            vidget2()
            if (go_error==11):
                canvas.create_text(ras*10.5,ras*7,text="белые выиграли!",font="Verdana 8",justify=CENTER,fill="black")
            elif (go_error==12):
                canvas.create_text(ras*10.5,ras*7,text="черные выиграли!",font="Verdana 8",justify=CENTER,fill="black")
            elif (control_vs==1 and go_color==2 or control_vs==2 and go_color==1 or control_vs==3):
                if (go_error==1):
                    canvas.create_text(ras*10.5,ras*7,text="вы должны есть",font="Verdana 8",justify=CENTER,fill="black")
                elif (go_error==2):
                    canvas.create_text(ras*10.5,ras*7,text="так ходить нельзя",font="Verdana 8",justify=CENTER,fill="black")
                elif (go_error==3):
                    canvas.create_text(ras*10.5,ras*7,text="сейчас ходит другой игрок",font="Verdana 7",justify=CENTER,fill="black")
        else:
            # остальные кнопки
            ...
            if (control_memory_mode==0):
                ...
                canvas.create_text(ras*10.5,ras*7.75,text="задания на память",font="Verdana 8",justify=CENTER,fill="black")
                canvas.create_rectangle(ras*9.25,ras*6.75,ras*11.75,ras*7.25,fill="white")
                canvas.create_text(ras*10.5,ras*7,text="дебюты",font="Verdana 8",justify=CENTER,fill="black")
                if (0):
                ...
                col = "black"
                if (control_memory_mode==1 or control_memory_mode==3):
                    col = "black"
                    canvas.create_rectangle(ras*9.25,ras*8.25,ras*11.75,ras*8.75,fill="white")
                ...
                if (control_memory_mode<3):   
                    ...
                    canvas.create_text(ras*10.5,ras*6.35,text="задание",font="Verdana 8",justify=CENTER,fill="grey")
                #canvas.create_rectangle(ras*9.25,ras*6.75,ras*11.75,ras*7.25,fill="white")
    ...
    def nostardamus(go_color):
        ...
        def helper(area1,H,go_color):
            ...
            while (len(go_operation_save)>i): #  and i<1
                ...
                go[len(go)-1].append(evklid(area2, go_color))
                mas = deepcopy(go[len(go)-1])
                if (int(len(go[len(go)-1])/2)<H):
                    helper(area2,H,more_color(go_color))
                go.append(deepcopy(mas))
                if (abs(go[len(go)-1][len(go[len(go)-1])-1])<1000000 and 1):
        ...
        go.clear()
        H = 2
        go.append(evklid(area_monitor, go_color)) # оценка в начале позиции 
        ...
        while (i<len(go_operation_save)): # записываем возможные ходы в данный момент времени, и каждый раз вызываем рекрусивно просчет helper
            ...
            go.append([go_operation_save[i],evklid(area_monitor1, go_color)])
            helper(area_monitor1,H,more_color(go_color))
            i = i + 1
        timer1 = millis()
        print("--------")
        print_mas(go)
    ...
    def evklid(area, go_color): # go_operation1 - возможные ходы первого ходящего
        global go_operation
        generator_move(area,go_color)
        if (len(go_operation)>0): 
            x = 0
    ...
    def click1(event):
        ...
        if (klik_flag==1):
            ...
            if ((x >= ras/2) and (x <= ras/2*17) and (y >= ras/2) and (y <= ras/2*17)): 
                ...
                if (control_edit==0):
                    if (control_vs==1 and go_color==2 or control_vs==2 and go_color==1 or control_vs==3): 
                        ...
                        go_error = 3
                        reboot_controller()
                elif (area_monitor[y][x] != 9): # штука контролирующая добавление шашек на поле (нажатия на поле)
                ...
                if (control_edit==0):
                    ...
                    if (x>=ras*9.25 and x<=ras*11.75):
                        ...
                        if (control_memory_mode==0): # режим для развития памяти
                            ...
                            if (control_memory_mode==1 and y>=ras*6.75 and y<=ras*7.25):
                                ...
                                if (control_memory_mode_i<1):
                                    control_memory_mode_i = 1
                            if (control_memory_mode==3 and y>=ras*6.75 and y<=ras*7.25):
                                if (x>=ras*9.25 and x<=ras*9.75):
                                    control_memory_mode_i = control_memory_mode_i - 10
                                if (x>=ras*9.75 and x<=ras*10.25):
                                    control_memory_mode_i = control_memory_mode_i - 1
            ...
            if (control_edit==0): # вызов рисовалки виджетов
                if (len(history)>1):
                    control_protocol.clear()
                    control_protocol_flag = len(history) - 1
                    i = 0
