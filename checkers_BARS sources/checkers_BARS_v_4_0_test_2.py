...
def qwerty():
    ...
    out_window = Tk()
    out_window.title('BARS 4.0')#============> поменять версию и в print_file() # demonstration special
    ...
    def print_file():
        ...
        file.write("\n")
        file.write("Протокол составлен программой BARS 4.0\n") # <========= поменяй версию!!!
        #
    ...
    def  reboot_controller():
        ...
        if (control_edit==0):
            ...
            vidget2()
            if (go_error==11):
                canvas.create_text(ras*10.5,ras*6.75,text="белые выиграли!",font="Verdana 8",justify=CENTER,fill="black")
            elif (go_error==12):
                canvas.create_text(ras*10.5,ras*6.75,text="черные выиграли!",font="Verdana 8",justify=CENTER,fill="black")
                ...
                if (control_edit==0):
                    ...
                    if (x>=ras*9.25 and x<=ras*11.75):
                        ...
                        if (control_memory_mode==0): # режим для развития памяти
                            #canvas.create_rectangle(ras*9.25,ras*7.75,ras*11.75,ras*8.125,fill="white") "задания на память"
                            #canvas.create_rectangle(ras*9.25,ras*7.25,ras*11.75,ras*7.625,fill="white") "дебюты"
                            #canvas.create_rectangle(ras*9.25,ras*7.125,ras*11.75,ras*6.75,fill="white") "задачник"
                            ...
                            # режим дебютов
                            if (y>=ras*7.25 and y<=ras*7.625): 
                                control_memory_mode = 3
                                control_memory_mode_i = 1
                                area_monitor = deepcopy(debuts[control_memory_mode_i][1])
                            # задачник
                            if (y>=ras*7.125 and y<=ras*6.75 and 0): 
