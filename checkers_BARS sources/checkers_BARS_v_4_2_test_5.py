...
def qwerty():
    ...
    out_window = Tk()
    out_window.title('BARS 4.2 teaching')#============> поменять версию и в print_file() # demonstration special
    ...
    def print_file():
        ...
        file.write("\n")
        file.write("Протокол составлен программой BARS 4.2 teaching\n") # <========= поменяй версию!!!
        #
    ...
    def  reboot_controller():
        ...
        if (control_edit==0 or control_memory_mode==6):
            ...
            if (go_error!=11 and go_error!=12):
                canvas.create_rectangle(ras*10.25,ras*7.25,ras*10.75,ras*6.75,fill="white", outline="black")
                col = "black"
                if (control_revers_monitor==1):
                    col = "grey"
                canvas.create_polygon(ras*10.4,ras*7.15,ras*10.3,ras*7.05,ras*10.35,ras*7.05,ras*10.35,ras*6.85,ras*10.43,ras*6.85,ras*10.43,ras*7.05,ras*10.5,ras*7.05, fill="white", outline=col)
                canvas.create_polygon(ras*10.5,ras*6.95,ras*10.6,ras*6.85,ras*10.7,ras*6.95,ras*10.63,ras*6.95,ras*10.63,ras*7.15,ras*10.55,ras*7.15,ras*10.55,ras*6.95, fill="white", outline=col)
            #if (control_revers_monitor==0):
            #    canvas.create_line(ras*10.3,ras*7.2,ras*10.7,ras*6.8,width=1,fill="black")
            #    canvas.create_line(ras*10.3,ras*6.8,ras*10.7,ras*7.2,width=1,fill="black")
        else:
            # остальные кнопки
            ...
            if ((x >= ras/2) and (x <= ras/2*17) and (y >= ras/2) and (y <= ras/2*17)): 
                ...
                if ((y>=ras*8.25 and y<=ras*8.75 and x>=ras*10.625 and x<=ras*11.75 and control_edit==0 or y>=ras*8.25 and y<=ras*8.75 and x>=ras*9.25 and x<=ras*11.75 and control_edit==1) and control_memory_mode==0):
                    ...
                    if (control_edit==1):
                        ...
                        go_color = -1
                    control_revers_monitor = 0
                elif (x>=ras*10.25 and y<=ras*7.25 and x<=ras*10.75 and y>=ras*6.75 and (control_edit==0 or control_memory_mode==6) and go_error!=11 and go_error!=12): 
                    if (control_revers_monitor==1):
                        control_revers_monitor = 0
                    else:
