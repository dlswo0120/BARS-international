...
def qwerty():
    ...
    out_window = Tk()
    out_window.title('BARS 4.1')#============> поменять версию и в print_file() # demonstration special
    ...
    def print_file():
        ...
        file.write("\n")
        file.write("Протокол составлен программой BARS 4.1\n") # <========= поменяй версию!!!
        #
    ...
    def  reboot_controller():
        ...
        if (control_edit==0 or control_memory_mode==6):
            ...
            if (go_error==11):
                ...
                if (go_error==1):
                    canvas.create_text(ras*10.5,ras*6.75,text="вы должны есть",font="Verdana 8",justify=CENTER,fill="black")
                elif (go_error==2):
                    canvas.create_text(ras*10.5,ras*6.75,text="так ходить нельзя",font="Verdana 8",justify=CENTER,fill="black")
                elif (go_error==3):
                    canvas.create_text(ras*10.5,ras*6.75,text="сейчас ходит другой игрок",font="Verdana 7",justify=CENTER,fill="black")
                elif (go_error==4):
                    canvas.create_text(ras*10.5,ras*6.75,text="подумай еще",font="Verdana 8",justify=CENTER,fill="black")
                elif (go_error==5):
                    canvas.create_text(ras*10.5,ras*6.75,text="задание выполнено!",font="Verdana 8",justify=CENTER,fill="black")
                elif (go_error==9):
                    canvas.create_text(ras*10.5,ras*6.75,text="робот думает...",font="Verdana 8",justify=CENTER,fill="black")
            # for special version:
            if (control_memory_mode!=6):
    ...
    def nostardamus(go_color):
        ...
        def helper(area1,go_color):
            ...
            def helper1():
                global go
                delta = 0
                if   (go[len(go)-2][len(go[len(go)-2])-1]<go[len(go)-3][len(go[len(go)-3])-1]):
                    del go[len(go)-2]
                elif (go[len(go)-2][len(go[len(go)-2])-1]>go[len(go)-3][len(go[len(go)-3])-1]):
                    del go[len(go)-3]
                    #helper1()
            ...
            while (len(go_operation_save)>i): #  and i<1
                ...
                go[len(go)-2].append(evklid_lite(area2))
        ...
        go_operation_save = deepcopy(go_operation)
        go.append(evklid_lite(area_monitor)) # оценка в начале позиции 
        i = 0
        timer = millis()
        ...
        if (len(go_operation_save)==1): # один вариант хода
            ...
            while ((millis()-timer)*4<time_for_mozg and H<10): # 
                go_save = deepcopy(go)
                go.clear()
                go.append(go_save[0])
                i = 1
                while (millis()-timer<time_for_mozg+5000 and i<len(go_save)):
                    go.append(go_save[i])
                    if (abs(go[len(go)-1][len(go[len(go)-1])-1])<999999):
                        area_monitor1 = going(area_monitor, go[len(go)-1][0])
                        u = 2
                        while (millis()-timer<time_for_mozg+5000 and u<len(go[len(go)-1])):
                            area_monitor1 = going(area_monitor1, go[len(go)-1][u])
                            u += 2
                        if (H%2==1):
                            helper(area_monitor1,more_color(go_color))
                        else:
                            helper(area_monitor1,go_color)
                        #print_mas(area_monitor1)
                        #print()
                    i += 1
                if (millis()-timer<time_for_mozg+5000 and H==3 and 0):
                    i = 1
                    go_save = deepcopy(go)
                    while (len(go)>i):
                        if (len(go[i][0])==2 and len(go[i][2])==2 and len(go[i][4])==2):
                            del go[i]
                        else:
                            i += 1
                    if (len(go)==1):
        ...
        if (0):
            ...
            if (H==2):
                ...
                if (H%2==1):
                    ii = 1000001
                while (i<len(go)):
                    if (go[i][len(go[i])-1]>ii and H%2==0 or go[i][len(go[i])-1]<ii and H%2==1):
                        ii = go[i][len(go[i])-1]
                    i = i + 1
                if (vivodit_ili_net):
                    print(ii)
                ...
                while (flag==0):
                    ...
                    while (i<len(go)):
                        ...
                        i = i + 1
                    i = 1
                    while (i<len(go) and flag==0):
                        if (go[i][len(go[i])-1]==ii):
                            if (pifagor(i)): # проверка на оптимальнсть данных действий
                                print("===>  ", go_operation)
                                go_operation = deepcopy(go[i][0])
                                flag = 1
                            else:
